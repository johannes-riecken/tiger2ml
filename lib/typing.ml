...
type mutable_flag = Immutable | Mutable of bool ref
type primitive_flag = User | Primitive
...
type loop_flag = InLoop of bool ref | NoLoop
...
let new_frame env = { env with in_loop = NoLoop }
...
let save_record_types env = function
  | [] -> ()
  | _ as typs -> env.record_types := typs :: !(env.record_types)

let find_type env (loc, name) =
  try SMap.find name env.types with Not_found -> error loc (TypeNotFound name)

let find_type_opt env (_, name) =
  try Some (SMap.find name env.types) with Not_found -> None

let rec actual_type = function
  | TForward r -> (
      match !r with None -> assert false | Some t -> actual_type t)
  | _ as x -> x

let find_var env (loc, name) =
  try match SMap.find name env.values with Var v -> v | _ -> raise Not_found
  with Not_found -> error loc (VariableNotFound name)

let find_record_type env (loc, name) =
  try
    match actual_type (SMap.find name env.types) with
    | TRecord (_, flds) as t -> (t, flds)
    | _ -> error loc RecordTypeExpected
  with Not_found -> error loc (TypeNotFound name)

let find_array_type env (loc, name) =
  try
    match actual_type (SMap.find name env.types) with
    | TArray t1 as t -> (t, t1)
    | _ -> error loc ArrayTypeExpected
  with Not_found -> error loc (TypeNotFound name)

let find_fun env (loc, name) =
  try match SMap.find name env.values with Fun f -> f | _ -> raise Not_found
  with Not_found -> error loc (FunctionNotFound name)
...
let add_fun env name args rty =
  ...
  ( name',
    {
      env with
      values = SMap.add name (Fun (name', args, rty, User)) env.values;
    } )
...
let add_prim env name args rty =
  ...
  {
    env with
    values = SMap.add name (Fun (name', args, rty, Primitive)) env.values;
  }

let add_type env (_, name) t = { env with types = SMap.add name t env.types }
let is_record_type t = match actual_type t with TRecord _ -> true | _ -> false

let rec var env = function
  ...
  | PIndexVar (_, v, index) -> (
      ...
      | _ -> error (loc_var v) ArrayExpected)
  | PFieldVar (_, v, (loc, name)) -> (
      ...
      | TRecord (_, flds) -> (
          ...
          with Not_found -> error loc (FieldNotFound name))
      | _ -> error (loc_var v) RecordExpected)

and int_exp env e =
  let t, e' = exp env e in
  match actual_type t with
  | TAny | TInt -> e'
  | TBool -> TIfExp (e', TIntExp 1, Some (TIntExp 0))
  | _ -> error (loc_exp e) IntExpected

and bool_exp env e =
  let t, e' = exp env e in
  match actual_type t with
  | TAny | TBool -> e'
  | TInt -> TCallExp ("<>", [ e'; TIntExp 0 ])
  | _ -> error (loc_exp e) IntExpected
...
and unit_exp env e =
  ...
  match actual_type t with
  | TAny | TUnit -> e'
  | _ -> error (loc_exp e) UnitExpected

and typ_exp env t e =
  let t', e' = exp env e in
  match (actual_type t, actual_type t') with
  | TBool, TInt -> TCallExp ("<>", [ e'; TIntExp 0 ])
  | TInt, TBool -> TIfExp (e', TIntExp 1, Some (TIntExp 0))
  | TBool, TBool
  | TInt, TInt
  | TUnit, TUnit
  | TString, TString
  ...
  | ((TRecord _ | TArray _) as t1), ((TRecord _ | TArray _) as t2) when t1 == t2
    ->
      e'
  | _ -> error (loc_exp e) TypeMismatch

and int_or_string_exp env e =
  let t, e' = exp env e in
  match actual_type t with
  | TAny | TInt | TBool | TString -> (t, e')
  | _ -> error (loc_exp e) IntOrStringExpected

and exp env = function
  | PIntExp (_, n) -> (TInt, TIntExp n)
  | PStringExp (_, s) -> (TString, TStringExp s)
  | PUnitExp _ -> (TUnit, TUnitExp)
  | PSeqExp (_, e1, e2) ->
      let _, e1 = exp env e1 in
      let t, e2 = exp env e2 in
      (t, TSeqExp (e1, e2))
  | PNilExp _ -> (TNil, TNilExp)
  | PBreakExp loc -> (
      match env.in_loop with
      | InLoop has_break ->
          has_break := true;
          (TAny, TBreakExp)
      | NoLoop -> error loc BadBreak)
  ...
  | PBinExp (_, e1, (("=" | "<>") as op), e2) -> (
      ...
      let e2 = typ_exp env t1 e2 in
      let op' =
        match op with "=" -> "==" | "<>" -> "!=" | _ -> assert false
      in
      match actual_type t1 with
      | TString | TInt | TBool | TAny | TUnit ->
          (TBool, TCallExp (op, [ e1; e2 ]))
      | TRecord _ | TArray _ -> (TBool, TCallExp (op', [ e1; e2 ]))
      | _ -> assert false)
  | PBinExp _ -> assert false
  | PUnaryExp (_, "-", e) ->
      let e = int_exp env e in
      (TInt, TCallExp ("~-", [ e ]))
  | PUnaryExp _ -> assert false
  | PIfExp (_, e1, e2, None) ->
      let e1 = bool_exp env e1 in
  ...
  | PAssignExp (_, v, e) ->
      ...
      | TNameVar (_, Mutable m) -> m := true
      | _ -> ());
      (TUnit, TAssignExp (rc, ec))
  | PVarExp (_, v) ->
      let rt, rc = var env v in
      (rt, TVarExp rc)
  | PLetExp (_, d, e) -> dec env d e
        ...
        List.map2
          (fun (f1n, f1tn) ((_, f2n), f2init) ->
            if f1n = f2n then
              ...
              (f2n, f2initc)
            else error (loc_exp f2init) BadFieldName)
          t1 fields
      in
      (t, TRecordExp flds)
...
and dec env d e =
  ...
  | PVarDec (_, ((_, name), Some t, init)) ->
      ...
      (t, TLetExp (name, init1, mf, e))
  | PTypeDec (_, typs) ->
      let rec scan env refs = function
        | [] -> (env, List.rev refs)
        | t :: typs ->
            let r = ref None in
      ...
      let mktype r = function
        | PRecordTyp (_, (_, id), flds) ->
            let flds =
              List.map (fun ((_, fid), tid) -> (fid, find_type env tid)) flds
            in
            save id flds;
            update r (TRecord (id, flds))
        | PArrayTyp (_, _, tid) -> update r (TArray (find_type env tid))
        | PNameTyp (_, _, tid) -> update r (find_type env tid)
      in
      ...
      let rec check seenr visited t =
        if List.memq t visited then
          ...
          | TArray t1 -> check seenr (t :: visited) t1
          | TForward r -> (
              match !r with
              | None -> assert false
              | Some t1 -> check seenr visited t1)
      in
      ...
      let funs', env1 =
        List.fold_left
          (fun (names', env) ((_, name), args, ret_typid, _) ->
            let rett =
              match ret_typid with None -> TUnit | Some t -> find_type env t
            in
            let name', env =
              add_fun env name
                (List.map (fun (_, y) -> find_type env y) args)
                rett
            in
            (name' :: names', env))
          ([], env) funs
            ...
            let env2, muts =
              List.fold_left
                (fun (env, muts) ((_, arg_name), arg_typid) ->
                  ...
                  let arg_name, env =
                    add_var env arg_name (find_type env arg_typid) mut
                  in
...
let std_types = [ ("int", TInt); ("string", TString) ]

let std_env () =
  let env = empty_env () in
  let env =
    List.fold_left
      (fun env (name, args, rty) -> add_prim env name args rty)
      env primitives
  in
  let env =
    List.fold_left
      (fun env (name, t) -> add_type env (Location.none, name) t)
      env std_types
  in
  env
