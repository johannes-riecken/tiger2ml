...
let mkident ?(loc = Location.none) s = Location.mkloc (Longident.parse s) loc

let rec emit_type = function
  | TInt -> Typ.constr (mkident "int") []
  | TString -> Typ.constr (mkident "string") []
  | TUnit -> Typ.constr (mkident "unit") []
  | TArray t -> Typ.constr (mkident "array") [ emit_type t ]
  | TAny -> assert false
  | TBool -> Typ.constr (mkident "bool") []
  | TRecord (id, _) ->
      Typ.constr (mkident "option") [ Typ.constr (mkident id) [] ]
  | TForward r -> (
      match !r with None -> assert false | Some t -> emit_type t)
  | TNil -> assert false
...
let rec expr = function
        ...
        | id, [], e ->
            {
              ...
              pvb_expr =
                Exp.function_
                  [
                    {
                      pc_lhs = Pat.construct (mkident "()") None;
                      pc_guard = None;
                      pc_rhs = expr e;
                    };
                  ];
                ...
                (fun (a, m) e ->
                  let e =
                    Exp.function_
                      [
                        {
                          pc_lhs = Pat.var (Location.mknoloc a);
                          pc_guard = None;
                          pc_rhs = e;
                        };
                      ]
                  ...
                  | Typing.Mutable m when !m ->
                      Exp.let_ Nonrecursive
                        [
                          {
                            pvb_pat = Pat.var (Location.mknoloc a);
                            pvb_expr =
                              Exp.apply
  ...
  | TAssignExp (TFieldVar (v, name, line), e) ->
      Exp.match_ (var v)
        [
          ...
          {
            pc_lhs =
              Pat.construct (mkident "Some")
                (Some (Pat.var (Location.mknoloc "x")));
            pc_guard = None;
            pc_rhs =
              Exp.setfield (Exp.ident (mkident "x")) (mkident name) (expr e);
          };
        ]
  ...
  | TForExp (index, start, finish, body, false) ->
      Exp.for_
        (Pat.var (Location.mknoloc index))
        (expr start) (expr finish) Upto (expr body)
  ...
  | TRecordExp fields ->
      ...
      Exp.construct (mkident "Some")
        (Some (Exp.record (List.map mkfield fields) None))
  | TIfExp (e1, e2, e3) -> Exp.ifthenelse (expr e1) (expr e2) (map_opt expr e3)
  | TWhileExp (e1, e2, false) -> Exp.while_ (expr e1) (expr e2)
