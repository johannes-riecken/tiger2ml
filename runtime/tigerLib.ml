...
let getchar () = try String.make 1 (input_char stdin) with End_of_file -> ""
let ord s = match String.length s with 0 -> -1 | _ -> int_of_char s.[0]
let chr n = try String.make 1 (char_of_int n) with _ -> failwith "chr"
let size = String.length

let substring s pos len =
  try String.sub s pos len with _ -> failwith "substring"
...
let get a i line =
  if i < 0 || i >= Array.length a then raise (Out_of_bounds line)
